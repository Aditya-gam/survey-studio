"""Core logic for the multi-agent literature-review assistant.

Built with the **AutoGen** AgentChat stack (>= v0.4). It exposes a single public
coroutine `run_survey_studio()` that drives a two-agent team:
...
async def run_survey_studio(
    ...
    task_prompt = (
        f"Conduct a literature review on **{topic}** and return exactly "
        f"{num_papers} papers."
    )
...
if __name__ == "__main__":
    ...
    async def _demo() -> None:
        async for line in run_survey_studio("Artificial Intelligence", num_papers=5):
            print(line)
